...
async function toggleFavorite(mealId, mealData) {
    try {
        ...
        if (response.ok) {
            const detailsMealId = mealDetails.querySelector('.addFavoritesDetails')?.dataset.id;

            if (!mealDetails.classList.contains('hidden') && detailsMealId === mealId) {
                handleMealClick(null, mealId);
            ...
            if (currentCategory) {
                fetchFilteredMeals(currentCategory);
            } else if (searchTerm) {
...
function displayMeals(meals, heading) {
    ...
    if (!meals || meals.length === 0) {
        errorContainer.textContent = 'No recipes found. Try a different search term or category.';
        ...
        meals.forEach(meal => {
            ...
            mealEl.innerHTML = `
                ...
                <div class="card-body">
                    ...
                    <p class="card-meta">
                       Time: ${mealMetaData.cookingTime} min - Calories: ${mealMetaData.calories} cal </p>
                    <div class="card-btns">
                        ...
                        >
                            ${isFav ? 'Remove' : 'Add to Favourite'} <i class="${isFav ? 'fas' : 'far'} fa-heart"></i>
    ...
    try {
        ...
        const mealsWithDetails = await Promise.all(
            (data.meals || []).slice(0, 6).map(async (meal) => {
                const detailResponse = await fetch(`${LOOKUP_URL}${meal.idMeal}`);
                const detailData = await detailResponse.json();
                return detailData.meals ? detailData.meals[0] : meal;
            })
        );
        displayMeals(mealsWithDetails.filter(m => m));
    } catch (error) {
        errorContainer.textContent = 'Failed to load category meals.';
        errorContainer.classList.remove('hidden');
    ...
    try {
        ...
        if (meal) {
            ...
            mealDetailsContent.innerHTML = `
                ...
                <div class="meal-details-fav-btn">
                    <button class="addFavoritesDetails" 
                        data-id="${mealId}" 
                        data-name="${meal.strMeal}" 
                        data-img="${meal.strMealThumb}" 
                        data-category="${meal.strCategory || "Uncategorized"}"
                        data-calories="${mealsData.calories}"
                        data-time="${mealsData.cookingTime}"
                    >
...
mealDetailsContent.addEventListener("click", async (e) => {
    const favBtn = e.target.closest(".addFavoritesDetails");
    ...
    if (favBtn) {
        ...
        const isNowFavorite = await toggleFavorite(mealId, mealData);
        
        const iconClass = isNowFavorite ? 'fas' : 'far';
        favBtn.innerHTML = `<i class="${iconClass} fa-heart"></i>`;
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const initialButton = document.querySelector(`.category-title[data-category="${defaultCategory}"]`);
    if (initialButton) {
        initialButton.classList.add('active');
    }

    fetchFilteredMeals(defaultCategory);
});